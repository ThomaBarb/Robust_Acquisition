...
ChannelManager::ChannelManager(const SignalParameters& params, 
    ...
    : params_(params), pcps_config_(pcps_config), tracking_config_(tracking_config)
{}

void ChannelManager::add_channel(int prn) {
    channels_.push_back(std::make_unique<Channel>(prn, params_, pcps_config_, tracking_config_));
